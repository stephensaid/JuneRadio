...
void paintTimeModeScreen() {
  ...
  con.element.BG_COLOUR == TFT_BLACK ? bg = "blk" : bg = "wht";
  // curTemp = String(current->temperature);
  ...
  if (((con.element.ALARM_1_HOUR >= 0) && (con.element.ALARM_1_HOUR <= 24)) ||
    ...
    tft.setTextColor(con.element.HIGHLIGHT_COLOUR, con.element.BG_COLOUR);
    tft.loadFont(f015r);
    tft.drawString("Alarm", 235, 170);                                          // Alarm not set
    tft.drawString("not set", 235, 190);
    tft.unloadFont();
  }
...
void topBar(topbar t) {
  ...
  if ( WiFi.status() == WL_CONNECTED ) {
    ...
    ypos = 4;
    String wifi_icon;
    con.element.BG_COLOUR == TFT_BLACK ? wifi_icon = "/icons/wifi_black.jpg" : wifi_icon = "/icons/wifi_white.jpg";
